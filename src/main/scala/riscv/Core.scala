  ...
  def apply(
    ...
    pipeline.addPlugins(
      Seq(
        ...
        prefetcher,
        new Cache(sets = 2, ways = 2, skews=1, backbone.filterIBus, Some(prefetcher), maxPrefetches = 2),
        new Cache(sets = 8, ways = 2, skews=1, backbone.filterDBus, cacheable = (_ >= 0x80000000L)),
